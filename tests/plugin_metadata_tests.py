...
class PluginMetadataTests(unittest.TestCase):
    def test_source_plugin_uses_binary_install_hooks_and_wrappers(self):
        ...
        self.assertEqual("${HELM_PLUGIN_DIR}/scripts/run.sh", commands[1]["command"])
        ...
        self.assertEqual("${HELM_PLUGIN_DIR}/scripts/install.sh", hooks["install"][1]["command"])
        self.assertEqual(["--update"], hooks["update"][1]["args"])
