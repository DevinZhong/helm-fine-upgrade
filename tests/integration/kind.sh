...
helm fine-upgrade doctor --output-format json > doctor.json
...
helm fine-upgrade state-check "$release_name" "$chart_path" \
  --namespace "$namespace" \
  --output-format json > state-clean.json
assert_json_summary state-clean.json runtime_drift 0
...
helm fine-upgrade state-check "$release_name" "$chart_path" \
  --namespace "$namespace" \
  --output-format json \
  --fail-on runtime_drift > state-drift.json
...
assert_json_summary state-drift.json runtime_drift 1

expect_fail_on update plan-drift.json
assert_json_summary plan-drift.json update 1
...
helm fine-upgrade adopt-plan "$release_name" "$adoption_chart_path" \
  --namespace "$namespace" \
  --output-format json > adopt-plan.json
assert_json_summary adopt-plan.json adoptable 1

# Mutating commands must not run in non-interactive CI without --yes.
set +e
helm fine-upgrade apply "$release_name" "$chart_path" \
  --namespace "$namespace" > apply-without-confirmation.log 2>&1
apply_exit_code=$?
set -e
